...
namespace Car_Express_Solution.Views.employeCollectionView{
    public partial class employeCollectionView : XtraUserControl {
        public employeCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Car_Express_Solution.ViewModels.employeCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Car_Express_Solution.employe,
