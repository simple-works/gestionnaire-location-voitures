...
namespace RCD_Project.Views.UtilisateurCollectionView{
    public partial class UtilisateurCollectionView : XtraUserControl {
        public UtilisateurCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RCD_Project.ViewModels.UtilisateurCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RCD_Project.Utilisateur,
