...
namespace RCD_Project.Views.ContratCollectionView{
    public partial class ContratCollectionView : XtraUserControl {
        public ContratCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RCD_Project.ViewModels.ContratCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RCD_Project.Contrat,
