...
namespace RCD_Project.Views.ReservationCollectionView{
    public partial class ReservationCollectionView : XtraUserControl {
        public ReservationCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<RCD_Project.ViewModels.ReservationCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as RCD_Project.Reservation,
